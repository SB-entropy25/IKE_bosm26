-- ============================================================
-- IKE ULTIMATE F1 TRIVIA — Unified Hub Database Schema
-- Run this in your Supabase SQL Editor (one-time setup)
-- Project: https://fbgtqcogaykvdtysqmsz.supabase.co
-- ============================================================

-- 1. REGISTERED USERS (hub-level participants)
CREATE TABLE IF NOT EXISTS hub_users (
  id          uuid DEFAULT gen_random_uuid() PRIMARY KEY,
  name        text NOT NULL,
  bits_id     text UNIQUE NOT NULL,
  registered_at timestamptz DEFAULT now()
);

-- 2. HUB SETTINGS (admin controls - single row)
CREATE TABLE IF NOT EXISTS hub_settings (
  id                      int PRIMARY KEY DEFAULT 1,
  speed_round_enabled     boolean DEFAULT true,
  strategy_round_enabled  boolean DEFAULT false,
  strategy_weight         int DEFAULT 50,   -- 0-100: % weight of strategy score in final tally
  speed_game_state        text DEFAULT 'waiting',  -- 'waiting' | 'playing' | 'ended'
  current_question_index  int DEFAULT -1,
  updated_at              timestamptz DEFAULT now()
);

-- Insert default row
INSERT INTO hub_settings (id, speed_round_enabled, strategy_round_enabled, strategy_weight, speed_game_state, current_question_index)
VALUES (1, true, false, 50, 'waiting', -1)
ON CONFLICT (id) DO NOTHING;

-- 3. SPEED ROUND SCORES (IKE Quiz)
...
-- 4. STRATEGY ROUND SCORES (F1 Strategy Game)
CREATE TABLE IF NOT EXISTS strategy_scores (
  ...
  race_status     text DEFAULT 'Racing',
  rank_grade      text DEFAULT '',
  decisions_count int DEFAULT 0,
  updated_at      timestamptz DEFAULT now(),
  UNIQUE(bits_id)
);

-- 5. SPEED ROUND QUESTIONS
CREATE TABLE IF NOT EXISTS questions (
  ...
  time_allotted  int DEFAULT 90,
  sort_order     int DEFAULT 0
);

-- Seed Speed Round Questions
...
-- ============================================================
-- RLS POLICIES (Allow public access for the game)
-- ============================================================

-- Hub Users
ALTER TABLE hub_users ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS "Allow public read/insert on hub_users" ON hub_users;
CREATE POLICY "Allow public read/insert on hub_users" ON hub_users FOR ALL TO public USING (true) WITH CHECK (true);

-- Hub Settings
ALTER TABLE hub_settings ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS "Allow public read/update on hub_settings" ON hub_settings;
CREATE POLICY "Allow public read/update on hub_settings" ON hub_settings FOR ALL TO public USING (true) WITH CHECK (true);

-- Speed Scores
ALTER TABLE speed_scores ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS "Allow public read/insert on speed_scores" ON speed_scores;
CREATE POLICY "Allow public read/insert on speed_scores" ON speed_scores FOR ALL TO public USING (true) WITH CHECK (true);

-- Strategy Scores
ALTER TABLE strategy_scores ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS "Allow public read/insert on strategy_scores" ON strategy_scores;
CREATE POLICY "Allow public read/insert on strategy_scores" ON strategy_scores FOR ALL TO public USING (true) WITH CHECK (true);

-- Questions
ALTER TABLE questions ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS "Allow public read on questions" ON questions;
CREATE POLICY "Allow public read on questions" ON questions FOR SELECT TO public USING (true);


-- ============================================================
-- STRATEGY ROUND (F1 NEW) - GAME DATA TABLES
-- ============================================================
...
-- 7. Race Events Table (Strategy Scenarios)
...
-- 9. Game Config Table (Master Sheet for Scores and Themes)
...
-- Enable Realtime for live updates (Uncomment if needed, but usually handled in Supabase Dashboard)
-- ALTER PUBLICATION supabase_realtime ADD TABLE hub_settings;
-- ALTER PUBLICATION supabase_realtime ADD TABLE speed_scores;
-- ALTER PUBLICATION supabase_realtime ADD TABLE strategy_scores;
-- ALTER PUBLICATION supabase_realtime ADD TABLE hub_users;
