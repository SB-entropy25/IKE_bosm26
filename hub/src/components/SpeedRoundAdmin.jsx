...
export function SpeedRoundAdmin({ onBack }) {
  ...
  useEffect(() => {
    ...
    channel.on('broadcast', { event: 'submit_answer' }, async ({ payload }) => {
      ...
      if (isSkipped) {
        earnedPoints = 0; // Speed round has no bonus for skipping, but avoids the -5 penalty
      } else {
        ...
        if (isCorrect) {
          ...
          earnedPoints = -5 // Penalty for wrong answer
        }
      } 
  ...
  const exportResultsToCSV = async () => {
    try {
      ...
      const combined = scoresData.map(score => {
        ...
        const stratRec = strategyData?.find(s => s.bits_id === score.bits_id)
        const speedScore = score.score || 0
        const strategyScore = stratRec?.score || 0
        ...
        {activeTab === 'roster' && (
           <div className="h-full bg-[#1a1d24] border border-[#272b35] rounded-2xl shadow-lg flex flex-col">
             <div className="p-6 border-b border-[#272b35] bg-[#161920] flex justify-between items-center">
                ...
                <div className="flex items-center gap-4">
                  <label className="flex items-center gap-2 cursor-pointer bg-[#232730] px-3 py-2 rounded-xl border border-[#2b303b]">
                    <input 
                      type="checkbox" 
                      checked={notifyOnDeduct}
                      onChange={(e) => setNotifyOnDeduct(e.target.checked)}
                      className="w-4 h-4 rounded bg-gray-900 border-gray-600 focus:ring-cyan-600 focus:ring-2 text-cyan-600"
                    />
                    <span className="text-xs text-gray-400 font-bold uppercase tracking-wide">Alert on Deduct</span>
                  </label>
                  <button 
